...
public class DogObjects2 {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < dogPark.length; i++) {//sadece eating methodunu cagirdik.Dog classinda zaten sout print var.
            dogPark[i].eating("Chicken");//dogPark[i].eating(food1);
        }
        for (Dog each : dogPark) {
            each.drinking("water");//each.drinking(drink1);
        }
        for (int i = 0; i < dogPark.length; i++) {
            dogPark[i].playing("football");
        }
        for (Dog each : dogPark) {
            each.studying("java");//each.studying(subject1);
        }
    }
}
/*Muhtar'in kodlari
